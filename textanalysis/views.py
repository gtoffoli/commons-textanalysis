...
from textanalysis.tbx import tbx_xml_2_dict, tbx_tsv_2_dict, tbx_languages, tbx_subjects, tbx_filter_by_language
...
# from: https://sashamaps.net/docs/resources/20-colors/ (made minor changes)
distinct_colors = [
  ...
  ['Teal', '#008080'], ['Lavender', '#e6beff'], ['Brown', '#9a6324'], ['Dark Beige', '#9f8c76'], ['Maroon', '#800000'],
...
def ajax_file_to_item(request: HttpRequest, file_key: str) -> HttpResponse:
    ...
    title, text = get_file_text(body, content_type, title=content_disposition)
    obj_type = 'file'
    obj_id = hashlib.sha256(text.encode('utf-8')).hexdigest()
...
def tbx_view(request, file_key='', obj_type='', obj_id='', url=''):
    ...
    if is_ajax(request):
        ...
        if extension == 'tbx':
            ...
            data['invalid'] = 'invalid file extension'
            return JsonResponse(data)
        tbx = tbx_dict['tbx']
